...
namespace StAlbertAssessment2
{
    public partial class DataController : Form
    {
        ...
        {
            DaPrescription.Update(dsStAlbert, "Prescriptions");
        }

        private void daPrescription_RowUpdated(object sender, OleDbRowUpdatedEventArgs e)
        {
            int newID = 0;
            OleDbCommand idCMD = new OleDbCommand("SELECT @@IDENTITY", ctnStAlbert);
            if (e.StatementType == StatementType.Insert)
            {
                newID = (int)idCMD.ExecuteScalar();
                e.Row["PrescriptionID"] = newID;
            }
        }
        public void UpdatePayment()
        {
            DaPayment.Update(dsStAlbert, "Payments");
        }
